...
constexpr std::array<std::size_t, 11> THREADS { 1, 2, 4, 8, 16, 24, 32, 48, 64,
	80, 96 };
...
void run_steal(std::size_t total_ops, std::size_t runs, std::size_t threads);

/** runs all bench iterations for the specified bench and queue */
template <typename Q, typename R>
void run_benches(std::string_view queue_name, bench::bench_type_t bench_type,
	std::size_t total_ops, std::size_t runs, thread_span_t threads_range,
	make_queue_ref_fn<Q, R> make_queue_ref);

/** runs the pairwise enqueue/dequeue benchmark */
template <typename Q, typename R>
void bench_pairwise(std::string_view queue_name, std::size_t total_ops,
	std::size_t runs, std::size_t threads,
	make_queue_ref_fn<Q, R> make_queue_ref);

/** runs the burst benchmarks */
template <typename Q, typename R>
void bench_bursts(std::string_view queue_name, std::size_t total_ops,
	std::size_t runs, std::size_t threads,
	make_queue_ref_fn<Q, R> make_queue_ref);

/** runs either the read-heavy or write-heavy benchmark */
template <typename Q, typename R>
void bench_reads_or_writes(std::string_view queue_name,
	bench::bench_type_t bench_type, std::size_t total_ops, std::size_t runs,
	std::size_t threads, make_queue_ref_fn<Q, R> make_queue_ref);

/** potentially extracts the alternative threads span from the argument vector
 */
thread_span_t
extract_thread_span(int argc, char *argv[6], std::array<std::size_t, 1> &res)
{
	if (argc < 6) {
...
main(int argc, char *argv[5])
{
	...
	auto alternative_thread_range
		= std::to_array({ static_cast<std::size_t>(0) });
	const auto threads
		= extract_thread_span(argc, argv, alternative_thread_range);

	const std::string_view queue_name { bench::display_str(queue_type) };

	switch (queue_type) {
	case bench::queue_type_t::LCR:
		run_benches<lcr_queue, lcr_queue_ref>(queue_name, bench_type, total_ops,
			runs, threads, [](auto &queue, auto thread_id) -> auto {
				return lcr_queue_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::LOO:
		run_benches<loo_queue, loo_queue &>(queue_name, bench_type, total_ops, runs,
			threads, [](auto &queue, auto) -> auto & { return queue; });
		break;
	case bench::queue_type_t::FAA:
		run_benches<faa_queue, faa_queue_ref>(queue_name, bench_type, total_ops,
			runs, threads, [](auto &queue, auto thread_id) -> auto {
				return faa_queue_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::FAA_V1:
		run_benches<faa_queue_v1, faa_queue_v1_ref>(queue_name, bench_type,
			total_ops, runs, threads, [](auto &queue, auto thread_id) -> auto {
				return faa_queue_v1_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::FAA_V2:
		run_benches<faa_queue_v2, faa_queue_v2_ref>(queue_name, bench_type,
			total_ops, runs, threads, [](auto &queue, auto thread_id) -> auto {
				return faa_queue_v2_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::FAA_V3:
		run_benches<faa_queue_v3, faa_queue_v3_ref>(queue_name, bench_type,
			total_ops, runs, threads, [](auto &queue, auto thread_id) -> auto {
				return faa_queue_v3_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::MSC:
		run_benches<msc_queue, msc_queue_ref>(queue_name, bench_type, total_ops,
			runs, threads, [](auto &queue, auto thread_id) -> auto {
				return msc_queue_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::SCQ2:
		run_benches<lscq2_queue, lscq2_queue_ref>(queue_name, bench_type, total_ops,
			runs, threads, [](auto &queue, auto thread_id) -> auto {
				return lscq2_queue_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::SCQD:
		run_benches<lscqd_queue, lscqd_queue_ref>(queue_name, bench_type, total_ops,
			runs, threads, [](auto &queue, auto thread_id) -> auto {
				return lscqd_queue_ref(queue, thread_id);
			});
		break;
	case bench::queue_type_t::YMC:
		run_benches<ymc_queue, ymc_queue_ref>(queue_name, bench_type, total_ops,
			runs, threads, [](auto &queue, auto thread_id) -> auto {
...
template <typename Q, typename R>
void
run_benches(std::string_view queue_name, bench::bench_type_t bench_type,
	std::size_t total_ops, std::size_t runs, thread_span_t threads_range,
	make_queue_ref_fn<Q, R> make_queue_ref)
{
	if (bench_type == bench::bench_type_t::PAIRS
		|| bench_type == bench::bench_type_t::BURSTS) {
		for (auto threads : threads_range) {
			...
			case bench::bench_type_t::PAIRS:
				bench_pairwise<Q, R>(queue_name, total_ops, runs, threads,
					make_queue_ref);
				break;
			case bench::bench_type_t::BURSTS:
				bench_bursts<Q, R>(queue_name, total_ops, runs, threads,
					make_queue_ref);
				break;
			default:
		...
		for (auto threads : threads_range) {
			...
			bench_reads_or_writes<Q, R>(queue_name, bench_type, total_ops, runs,
				threads, make_queue_ref);
		}
	}
}

void
run_steal(std::size_t total_ops, std::size_t runs, std::size_t threads)
{
	...
	/*for (auto run = 0; run < runs; run++) {
		lcd::deque<std::size_t> deque { threads };
		std::barrier barrier { }
	}*/
}
...
template <typename Q, typename R>
void
bench_pairwise(std::string_view queue_name, std::size_t total_ops,
	std::size_t runs, std::size_t threads, make_queue_ref_fn<Q, R> make_queue_ref)
{
	...
	for (auto run = 0; run < runs; ++run) {
		...
		std::cout << queue_name << "," << threads << "," << duration.count() << ","
							<< total_ops << std::endl;
	}
}
...
template <typename Q, typename R>
void
bench_bursts(std::string_view queue_name, std::size_t total_ops,
	std::size_t runs, std::size_t threads, make_queue_ref_fn<Q, R> make_queue_ref)
{
	const auto ops_per_threads = total_ops / threads;
	...
	for (auto run = 0; run < runs; ++run) {
		...
		// print measurements to stdout
		std::cout << queue_name << "," << threads << "," << enq.count() << ","
							<< deq.count() << "," << total_ops << std::endl;
	}
}

template <typename Q, typename R>
void
bench_reads_or_writes(std::string_view queue_name,
	bench::bench_type_t bench_type, std::size_t total_ops, std::size_t runs,
	std::size_t threads, make_queue_ref_fn<Q, R> make_queue_ref)
{
	if (threads % 4 != 0) {
