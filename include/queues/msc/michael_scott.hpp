...
/* basically the original algorithm */
/*template <typename T>
void queue<T>::enqueue(queue::pointer elem, std::size_t thread_id)
{
	...
	while (true)
	{
		...
		if (this->m_tail.load(relaxed) != tail)
			continue;
		...
		if (next == nullptr)
		{
			if (tail->cas_next(next, node, release))
			{
...
template <typename T>
void
queue<T>::enqueue(queue::pointer elem, std::size_t thread_id)
{
	if (elem == nullptr) {
	...
	auto node = new node_t { elem };
	while (true) {
		auto tail = this->m_hazard_pointers.protect_ptr(this->m_tail.load(relaxed),
...
queue<T>::dequeue(std::size_t thread_id)
{
	auto head
		= this->m_hazard_pointers.protect(this->m_head, thread_id, HP_DEQ_HEAD);

	while (head != this->m_tail.load(relaxed)) {
		auto next
			= this->m_hazard_pointers.protect(head->next, thread_id, HP_DEQ_NEXT);
		if (this->cas_head(head, next, relaxed)) {
			auto res = next->elem;
...
template <typename T>
bool
queue<T>::cas_head(queue::node_t *curr, queue::node_t *next,
...
template <typename T>
bool
queue<T>::cas_tail(queue::node_t *curr, queue::node_t *next,
	std::memory_order order)
{
...
template <typename T>
bool
queue<T>::node_t::cas_next(queue::node_t *&curr, queue::node_t *next_node,
	std::memory_order order)
{
