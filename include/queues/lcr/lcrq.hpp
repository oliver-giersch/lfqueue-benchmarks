...
struct queue<T>::crq_node_t {
	...
	bool
	cas_next(crq_node_t *expected, crq_node_t *desired, std::memory_order order)
	{
		return this->next.compare_exchange_strong(expected, desired, order,
			relaxed);
	}
};

template <typename T>
queue<T>::queue(std::size_t max_threads) : m_hazard_pointers { max_threads }
{
...
queue<T>::dequeue(std::size_t thread_id)
{
	pointer res;
	while (true) {
		auto head = this->m_hazard_pointers.protect_ptr(this->m_head.load(relaxed),
