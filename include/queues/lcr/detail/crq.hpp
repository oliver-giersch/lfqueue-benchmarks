...
namespace detail {
	...
	struct alignas(CACHE_LINE_SIZE) atomic_cell_t {
		...
		bool
		compare_exchange_weak(cell_t &expected, cell_t desired,
...
class queue<T>::crq_t {
	...
	static constexpr auto STATUS_BIT
		= std::uint64_t { 1 } << std::uint64_t { 63 };
...
template <typename T>
queue<T>::crq_t::crq_t() noexcept : m_head_ticket { 0 }
																	, m_tail_ticket { 0 }
{
	this->init_cells();
}
...
template <typename T>
queue<T>::crq_t::crq_t(pointer first) : m_head_ticket { 0 }
																			, m_tail_ticket { 1 }
{
	if (first == nullptr) {
...
template <typename T>
bool
queue<T>::crq_t::try_enqueue(pointer elem) noexcept
{
	auto attempts = 0;
	while (true) {
		const auto [is_closed, tail_ticket]
			= decomposed_idx_t { this->m_tail_ticket.fetch_add(1) };
		...
		if (ptr == nullptr) {
			if (idx <= tail_ticket
				...
				const auto desired
					= cell_t { decomposed_idx_t { STATUS_BIT, tail_ticket }.compose(),
							elem };
		...
		const auto cmp = static_cast<std::int64_t>(tail_ticket)
				- static_cast<std::int64_t>(head_ticket)
			>= RING_SIZE;
...
template <typename T>
bool
queue<T>::crq_t::try_dequeue(pointer &result) noexcept
{
	while (true) {
		...
		while (true) {
			...
			if (ptr != nullptr) {
				if (idx == head_ticket) {
					...
					auto expected
						= cell_t { decomposed_idx_t { is_safe, head_ticket }.compose(),
								ptr };
					...
					const auto desired
						= cell_t { decomposed_idx_t { 0, idx }.compose(), ptr };

					if (cell.compare_exchange_weak(expected, desired)) {
						break;
					}
				}
			} else { // attempt empty transition (idx <= head_ticket and ptr ==
							 // nullptr)
				auto expected = cell_t { composed_idx, nullptr };
				const auto desired = cell_t {
					decomposed_idx_t { is_safe, head_ticket + RING_SIZE }.compose(),
		...
		const auto tail_ticket
			= decomposed_idx_t { this->m_tail_ticket.load() }.idx;
