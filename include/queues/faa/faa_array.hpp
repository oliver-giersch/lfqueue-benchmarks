...
template <typename T, detail::queue_variant_t V>
void
queue<T, V>::enqueue(queue::pointer elem, std::size_t thread_id)
{
...
template <typename T, detail::queue_variant_t V>
typename queue<T, V>::pointer
queue<T, V>::dequeue(std::size_t thread_id)
{
	pointer res = nullptr;
	while (true) {
		...
		if (idx < NODE_SIZE) [[likely]] {
			...
			res
				= head->slots[idx].exchange(reinterpret_cast<pointer>(TAKEN), acquire);
...
template <typename T, detail::queue_variant_t V>
bool
queue<T, V>::is_empty(queue::node_t *head)
{
	if constexpr (V == detail::queue_variant_t::ORIGINAL) {
		return head->deq_idx.load(relaxed) >= head->enq_idx.load(acquire)
			&& head->next.load(acquire) == nullptr;
	} else if constexpr (V == detail::queue_variant_t::VARIANT_1) {
		return head->enq_idx.load(relaxed) <= head->deq_idx.load(acquire)
			&& head->next.load(acquire) == nullptr;
	} else if constexpr (V == detail::queue_variant_t::VARIANT_2) {
		return head->enq_idx.load(relaxed) <= head->deq_idx.fetch_add(0, acquire)
			&& head->next.load(acquire) == nullptr;
	} else {
		return head->deq_idx.fetch_add(0, relaxed) >= head->enq_idx.load(acquire)
			&& head->next.load(acquire) == nullptr;
	}
}
...
template <typename T, detail::queue_variant_t V>
bool
queue<T, V>::cas_head(queue::node_t *expected, queue::node_t *desired,
	std::memory_order order)
{
	return this->m_head.compare_exchange_strong(expected, desired, order,
		relaxed);
}

template <typename T, detail::queue_variant_t V>
bool
queue<T, V>::cas_tail(queue::node_t *expected, queue::node_t *desired,
	std::memory_order order)
{
	return this->m_tail.compare_exchange_strong(expected, desired, order,
		relaxed);
}
}
