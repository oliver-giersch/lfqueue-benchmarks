...
struct queue<T, V>::node_t {
	using slot_array_t
		= std::array<std::atomic<queue::pointer>, queue::NODE_SIZE>;
	...
	bool
	cas_slot_at(std::size_t idx, queue::pointer expected, queue::pointer desired,
		std::memory_order order)
	{
		return this->slots[idx].compare_exchange_strong(expected, desired, order,
