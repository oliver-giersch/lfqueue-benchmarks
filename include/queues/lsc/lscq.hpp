...
namespace detail {
	...
	struct node_t {
		...
		bool
		cas_next(node_t *expected, node_t *desired, std::memory_order order)
		{
...
template <typename T, template <typename> typename N>
void
queue<T, N>::enqueue(pointer elem, std::size_t thread_id)
{
	if (elem == nullptr) {
	...
	while (true) {
		auto tail = this->m_hazard_pointers.protect(this->m_tail.load(relaxed),
...
template <typename T, template <typename> typename N>
typename queue<T, N>::pointer
queue<T, N>::dequeue(std::size_t thread_id)
{
	pointer result;
	while (true) {
