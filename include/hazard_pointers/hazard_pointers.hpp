...
namespace memory {
template <typename T>
hazard_pointers<T>::hazard_pointers(std::size_t num_threads,
	std::size_t num_hazard_pointers, std::size_t scan_threshold)
...
template <typename T>
void
hazard_pointers<T>::clear_one(std::size_t thread_id, std::size_t hp)
{
	auto &thread_block = this->m_thread_blocks[thread_id];
	thread_block.hazard_ptrs[hp].ptr.store(nullptr, release);
}

template <typename T>
void
hazard_pointers<T>::clear(std::size_t thread_id)
{
	auto &thread_block = this->m_thread_blocks[thread_id];
	for (auto &hazard_ptr : thread_block.hazard_ptrs) {
		hazard_ptr.ptr.store(nullptr, relaxed);
	}
...
template <typename T>
hazard_pointers<T>::pointer
hazard_pointers<T>::protect(const std::atomic<hazard_pointers::pointer> &atomic,
	std::size_t thread_id, std::size_t hp)
{
	auto &hazard_ptr = this->m_thread_blocks[thread_id].hazard_ptrs[hp];
	auto curr = atomic.load(relaxed);
	while (true) {
		hazard_ptr.ptr.store(curr, seq_cst);
		const auto temp = atomic.load(acquire);
		if (curr == temp) {
			return curr;
		}
...
template <typename T>
hazard_pointers<T>::pointer
hazard_pointers<T>::protect_ptr(hazard_pointers::pointer ptr,
	std::size_t thread_id, std::size_t hp)
{
	auto &hazard_ptr = this->m_thread_blocks[thread_id].hazard_ptrs[hp];
	hazard_ptr.ptr.store(ptr, seq_cst);
...
template <typename T>
void
hazard_pointers<T>::retire(hazard_pointers::pointer ptr, std::size_t thread_id)
{
	auto &thread_retired_objects
		= this->m_thread_blocks[thread_id].retired_objects;
...
template <typename T>
bool
hazard_pointers<T>::can_reclaim(pointer retired) const
{
